  ...
  return (
    <>
      ...
      <div className="bg-white">
        {/* Hero */}
        <div className="relative isolate overflow-hidden bg-gradient-to-b from-teal-700 to-teal-800 pt-14">
           ...
           <div className="mx-auto container px-4 py-24 sm:py-32 lg:py-40">
            ...
            >
              <div className="inline-flex items-center rounded-full bg-teal-500/10 px-3 py-1 text-sm font-semibold leading-6 text-teal-300 ring-1 ring-inset ring-teal-500/20">
                {content.badge}
              </div>
              <h1 className="mt-6 text-4xl font-bold tracking-tight text-white sm:text-6xl">{content.title}</h1>
              <p className="mt-6 text-lg leading-8 text-teal-200">{content.subtitle}</p>
              <div className="mt-10 flex items-center gap-x-6">
                <Button size="lg" onClick={() => navigate('/contacto')}>
                    {content.cta.owner}
                </Button>
                <Button size="lg" variant="outline" className="text-white border-white hover:bg-white hover:text-teal-800" onClick={() => navigate('/para-clinicas/referir-caso')}>
        ...
        <Section className="bg-white">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl">{content.description.title}</h2>
              {content.description.paragraphs.map((p, i) => (
                <p key={i} className="mt-6 text-lg leading-8 text-slate-600">{p}</p>
              ))}
            </div>
             <div className="relative">
                <img alt={serviceImage.alt} className="w-full h-auto rounded-xl shadow-2xl" src={serviceImage.src} />
                <div className="absolute -bottom-4 -right-4 bg-teal-600 text-white p-4 rounded-lg shadow-lg w-48 text-center">
        ...
        <Section className="bg-slate-50">
          <div className="mx-auto max-w-2xl lg:text-center">
            <h2 className="text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl">{content.benefits.title}</h2>
          </div>
            ...
            <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-2 lg:max-w-none lg:grid-cols-3">
              {content.benefits.items.map((benefit, i) => {
                 ...
                 return (
                    ...
                    >
                      <IconComponent className="h-8 w-8 mb-2 text-teal-600" />
                      <dt className="text-lg font-semibold leading-7 text-slate-800">{benefit.text}</dt>
                    </div>
                 );
              })}
            </dl>
        ...
        <Section className="bg-white">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                <div className="order-2 lg:order-1">
                    <h2 className="text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl">{content.process.title}</h2>
                    ...
                    <ul className="mt-8 space-y-4">
                        {content.process.equipment.map((item, i) => (
                            <li key={i} className="flex items-center gap-x-3">
                                <Icons.CheckCircle className="h-6 w-6 flex-none text-teal-600" aria-hidden="true" />
                                <span className="text-md font-medium text-slate-700">{item}</span>
                            </li>
                        ))}
                    </ul>
                </div>
        ...
        <Section className="bg-slate-50">
            <div className="max-w-3xl mx-auto">
                <h2 className="text-center text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl">{content.faqs.title}</h2>
                <Accordion type="single" collapsible className="w-full mt-10">
                {content.faqs.items.map((faq, i) => (
                    <AccordionItem key={i} value={`item-${i}`} className="border-b-teal-200">
                        ...
                        <AccordionContent className="text-base text-slate-600 pt-2">
                            {faq.answer}
                        </AccordionContent>
                    </AccordionItem>
                ))}
