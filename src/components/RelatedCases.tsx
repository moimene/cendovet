  ...
  return (
    <div className="py-16 sm:py-24 bg-slate-50 dark:bg-slate-800/50">
      <div className="container mx-auto px-4">
        ...
        <div className="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-slate-200 dark:border-slate-700 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {relatedPosts.map((post, index) => (
            <motion.article
              ...
              className="flex max-w-xl flex-col items-start justify-between bg-white dark:bg-slate-900 p-6 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300"
            >
              <div>
                <div className="flex items-center gap-x-4 text-xs">
                    <time dateTime={post.date} className="text-slate-500 dark:text-slate-400 flex items-center">
                ...
                <div className="group relative">
                  <h3 className="mt-3 text-lg font-semibold leading-6 text-cyan-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400">
                    <Link to={`/blog/${post.slug}`}>
                      <span className="absolute inset-0" />
                      {post.title}
                    </Link>
                  </h3>
                  <p className="mt-5 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{post.summary}</p>
                </div>
               ...
               <div className="mt-6">
                 <Button variant="link" asChild className="p-0 text-cyan-600 dark:text-cyan-400 hover:text-cyan-800 dark:hover:text-cyan-200">
